...
void bmUpdateControl(){
  ...
  if(bmFuncMode && bmFuncSel==7){
    ...
    if(padHeld){
      ...
      if(!bmDrumPotLock[0] && bmPotCut!=bmDrumPitch[d]){ bmDrumPitch[d]=bmPotCut; bmApplyDrum(d); ch=true; }
      if(bmDrumPotLock[1]){ if(abs((int)bmPotRes-(int)bmDrumPotRef[1])>6) bmDrumPotLock[1]=false; }
      if(!bmDrumPotLock[1] && bmPotRes!=bmDrumLen[d]){ bmDrumLen[d]=bmPotRes; bmApplyDrum(d); ch=true; }
...
void bmDoDraw(){
  ...
  if(bmVsValDirty)      {bmVsValDirty=false; if(bmFuncMode&&bmFuncSel!=255) bmDrawFuncValue(); return;}
